...
H3K27ac seems to intersect most with CREB1, then CREB3L1, CREB3 and barely with CREB5.
...
```{r}
plotEnrichedHeatmaps(m_creb1_cf[1:6]) + plotEnrichedHeatmaps(m_creb1_cf[7:9])
```
...
An even better way to do this is to first estimate the background noise based on the whole genome and then use those to normalize our matrices (2b). Let's try both and compare.

```{r}
# Approach 2a
plotEnrichedHeatmaps(renormalizeBorders(m_creb1_cf), row_title = "Approach 2a")
#Approach 2b
nf <- bwNormFactors(pv_list_cf)
m_crebl_cf_normalized <- rescaleSignalMatrices(m_creb1_cf, nf)
plotEnrichedHeatmaps(m_crebl_cf_normalized, row_title = "Approach 2b0")
```

Gives me an error but also 2 heatmaps???
