...
Find what's enriched in one cluster with respect to the others (first we use cluster 1):
...
job_1 <- submitGreatJob(split_regions[["1"]], merged_peaks, species="hg38")
#Human GRCh38/hg38 is the genome we used
job_1
# we fetch the data back
res_1 <- getEnrichmentTables(job_1)
names(res_1)
```
...
## Plotting the top Biological Processes, molecular functions and cellular components of Cluster 1

```{r, fig.width=9, fig.height=6}
# looking at the biological processes
bp <- res_1$`GO Biological Process`
head(bp)
...
mf <- res_1$`GO Molecular Function`
head(mf)
...
cc <- res_1$`GO Cellular Component`
...
Now we only did this for cluster 3 compared to all clusters. We need to do more comparisons like this and figure out what cluster stands for what.
...
### Do another Cluster - Cluster 10

Now we do the same analysis for cluster 10.

```{r}
job_10 <- submitGreatJob(split_regions[["10"]], merged_peaks, species="hg38")
#Human GRCh38/hg38 is the genome we used
job_10
# we fetch the data back
res_10 <- getEnrichmentTables(job_10)
names(res_10)
```

## Plotting the top Biological Processes, molecular functions and cellular components of Cluster 10

```{r, fig.width=9, fig.height=6}
# looking at the biological processes
bp <- res_10$`GO Biological Process`
head(bp)
# reorder it from most to least statistically significant before being alphabetically ordered.
ggplot(head(bp,20), aes(Hyper_Fold_Enrichment, reorder(name, Hyper_Adjp_BH), size=Hyper_Foreground_Region_Hits, color=-log10(Hyper_Adjp_BH))) + geom_point() + scale_color_viridis_c()

# looking at the molecular function
mf <- res_10$`GO Molecular Function`
head(mf)
ggplot(head(mf,20), aes(Hyper_Fold_Enrichment, reorder(name, Hyper_Adjp_BH), size=Hyper_Foreground_Region_Hits, color=-log10(Hyper_Adjp_BH))) + geom_point() + scale_color_viridis_c()

# looking at cellular components
cc <- res_10$`GO Cellular Component`
ggplot(head(cc,20), aes(Hyper_Fold_Enrichment, reorder(name, Hyper_Adjp_BH), size=Hyper_Foreground_Region_Hits, color=-log10(Hyper_Adjp_BH))) + geom_point() + scale_color_viridis_c()
```
